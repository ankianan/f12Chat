...
(function () {
	...
	var noStorage = process && process.env && "production" === 'production' ? function () {
	  /* noop */return null;
	} : function () {
	...
	function createAsyncLocalStorage (type, config) {
...
var 	funcTag$1 = '[object Function]';
	...
	function isFunction(value) {
	  ...
	  return tag == funcTag$1 || tag == genTag || tag == proxyTag;
	}
...
var 	objectTag$2 = '[object Object]';
	...
	function isSymbol(value) {
	  return typeof value == 'symbol' ||
	    (isObjectLike(value) && objectToString$6.call(value) == symbolTag$1);
	}
...
var 	PARTIAL_COMPARE_FLAG$5 = 2;
	...
	function createPersistor(store, config) {
	  ...
	  function adhocRehydrate(incoming) {
	    ...
	    var state = {};
	    if (options.serial) {
	      forEach(incoming, function (subState, key) {
	        try {
	...
	function getStoredState(config, onComplete) {
	  var storage = config.storage || createAsyncLocalStorage('local');
	  var deserialize = config.deserialize || defaultDeserialize$1;
	  var blacklist = config.blacklist || [];
	  var whitelist = config.whitelist || false;
	  var transforms = config.transforms || [];
	  var keyPrefix = config.keyPrefix !== undefined ? config.keyPrefix : KEY_PREFIX;
	  ...
	  storage.getAllKeys(function (err, allKeys) {
	    ...
	    forEach(keysToRestore, function (key) {
	...
	function defaultDeserialize$1(serial) {
	  return JSON.parse(serial);
	}
	...
	function persistStore(store) {
	  ...
	  if ("production" !== 'production' && config.skipRestore) {}
	  ...
	  // restore
	  if (shouldRestore) {
	    genericSetImmediate$1(function () {
	  ...
	  } else genericSetImmediate$1(complete);
	...
	var REHYDRATE$1 = constants.REHYDRATE;
	...
	*/
	...
	var Redux$1 = window.interfaces.Redux;

	var reducer = Redux$1.combineReducers({
	    "route": reducer$1,
	    "account": reducer$4,
	    "user": reducer$3,
	    "contacts": reducer$2
	...
	var page$3 = _window$interfaces$3.page;
	...
	VirtualDom.render(Virtual.createElement(Root, null), document.getElementById("root"));
	//});
